...
export default function ClinicDetails() {
    ...
    // Derive doctors and services from the unified clinic object
    // (API returns clinic_services and doctors as nested Supabase relational arrays)
    const doctors = clinic?.doctors ?? [];
    const services = clinic?.clinic_services?.map(s => s.service_name) ?? [];

    const [bookingForm, setBookingForm] = useState({ doctorId: '', date: '', shift: '' });
    ...
    const handleBookNow = () => {
        if (!bookingForm.doctorId) { alert('يرجى اختيار الطبيب'); return; }
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            onChange={(e) => setBookingForm({ ...bookingForm, doctorId: e.target.value, shift: '' })}
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            {availableShifts.map((shift, idx) => (
                                                <option key={idx} value={shift.label}>{shift.label}</option>
                                            ))}
                                        </select>
