 ...
 * Currently manages: WhatsApp button (number + active toggle).
...
export default function SettingsCMS() {
  ...
  const handleSave = async () => {
    if (!whatsappNumber.trim()) { setError('رقم الواتساب مطلوب'); return; }
    setSaving(true);
    setError('');
    setSaved(false);

    const { data, error: err } = await updateSiteSettings({
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
        ...
        {saved && (
          <p className="flex items-center gap-2 text-green-600 text-xs bg-green-50 border border-green-100 rounded-lg px-3 py-2">
            <CheckCircle size={13} /> تم حفظ الإعدادات بنجاح!
          </p>
        )}
