...
export default function WhatsAppButton() {
  // رقم الواتساب مع المفتاح الدولي (بدون أصفار أو علامة +)
  const phoneNumber = "967777552666"; 
  
  // رسالة افتراضية تظهر للمستخدم عند فتح الواتساب (اختياري)
  ...
  return (
    <a
      ...
      // الكلاسات هنا تجعل الزر عائماً في الزاوية اليسرى السفلية ليتناسب مع المواقع العربية (RTL)
    ...
    >
      {/* Tooltip تظهر عند تمرير الماوس (في الكمبيوتر) */}
      <span className="absolute left-16 md:left-20 w-max bg-white text-gray-800 text-xs md:text-sm font-bold py-2 px-4 rounded-2xl shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none border border-gray-100">
        تواصل معنا
        <div className="absolute top-1/2 -left-2 transform -translate-y-1/2 border-8 border-transparent border-r-white"></div>
      </span>
